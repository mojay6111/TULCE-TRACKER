...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${map[status]}`}>{status}</span>
  );
}
...
export default function CustomerDetailPage() {
  ...
  const formatDate = (d) => new Date(d + 'T00:00:00').toLocaleDateString('en-KE', {
    weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'
  });
  const formatTime = (dt) => new Date(dt).toLocaleTimeString('en-KE', { hour: '2-digit', minute: '2-digit' });
  ...
  if (!customer) return <div className="flex items-center justify-center h-full text-gray-500">Customer not found.</div>;
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto animate-fade-in">
      {/* Back */}
      <button onClick={() => navigate('/customers')} className="text-gray-500 hover:text-gray-300 text-sm mb-4 flex items-center gap-1 transition-colors">
        ← Back to Customers
      </button>
      ...
      <div className="bg-dark-800 border border-dark-600 rounded-xl p-5 mb-5">
        <div className="flex items-start justify-between">
          ...
          <button onClick={handleDeleteCustomer} className="text-xs text-red-500 hover:text-red-400 transition-colors px-3 py-1.5 rounded-lg hover:bg-red-900/20">
            Delete Customer
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="bg-dark-700 rounded-lg p-3 text-center">
            ...
            <p className="font-display text-lg font-bold text-white">{formatKES(customer.totalOwed)}</p>
          </div>
          <div className="bg-dark-700 rounded-lg p-3 text-center">
            <p className="text-xs text-gray-500 mb-1">Total Paid</p>
            <p className="font-display text-lg font-bold text-emerald-400">{formatKES(customer.totalPaid)}</p>
          </div>
          <div className={`rounded-lg p-3 text-center ${customer.debt > 0 ? 'bg-red-900/20 border border-red-800/40' : 'bg-emerald-900/20 border border-emerald-800/40'}`}>
            <p className="text-xs text-gray-500 mb-1">Outstanding</p>
            <p className={`font-display text-lg font-bold ${customer.debt > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
            ...
            return (
              <div key={day} className="bg-dark-800 border border-dark-600 rounded-xl overflow-hidden">
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-dark-700 bg-dark-700/50">
                  <div>
                    <p className="text-sm font-display font-semibold text-white">{formatDate(day)}</p>
                    <p className="text-xs text-gray-500">{orders.length} order{orders.length !== 1 ? 's' : ''}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-500">Day total: {formatKES(dayTotal)}</p>
                    {dayDebt > 0 && <p className="text-xs text-red-400">Unpaid: {formatKES(dayDebt)}</p>}
                    {dayDebt <= 0 && <p className="text-xs text-emerald-400">✓ Fully paid</p>}
                  </div>
                </div>
                ...
                <div className="divide-y divide-dark-700">
                  {orders.map(order => (
                    <div key={order.id} className="px-4 py-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="text-sm text-white font-medium">
                              {order.quantity} tulce{order.quantity !== 1 ? 's' : ''} × KES 5 = {formatKES(order.totalAmount)}
                            </p>
                            <StatusBadge status={order.status} />
                          </div>
                          <p className="text-xs text-gray-500 mt-0.5">{formatTime(order.date)}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          {order.balance > 0 && (
                            <button
                        ...
                        <div className="mt-2 space-y-1">
                          {order.payments.map(p => (
                            <div key={p.id} className="flex items-center gap-2 text-xs text-gray-500">
                              <span className={p.method === 'MPESA' ? 'text-green-500' : 'text-blue-400'}>
                      ...
                      {order.balance > 0 && (
                        <p className="text-xs text-red-400 mt-1">Balance: {formatKES(order.balance)}</p>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {selectedOrder && (
        <PaymentModal
          ...
          onSuccess={() => { setSelectedOrder(null); fetchCustomer(); }}
        />
      )}
    </div>
  );
